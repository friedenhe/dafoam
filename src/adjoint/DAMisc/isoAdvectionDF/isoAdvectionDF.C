/*---------------------------------------------------------------------------*\
    ...
    Description:
        A customized version of isoAdvection class
...
namespace Foam
{
defineTypeNameAndDebug(isoAdvectionDF, 0);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::isoAdvectionDF::isoAdvectionDF(
    ...
    : // General data
      regIOobject(
          IOobject(
              "isoAdvectionDF", // the db name
              alpha1.mesh().time().timeName(),
              alpha1.mesh(), // register to mesh
              IOobject::NO_READ,
              IOobject::NO_WRITE,
...
void Foam::isoAdvectionDF::timeIntegratedFlux(surfaceScalarField& dVf)
{
    // Get time step
    ...
    scalarField& dVfIn = dVf.primitiveFieldRef();
    ...
    surfaceScalarField::Boundary& dVfb = dVf.boundaryFieldRef();
...
void Foam::isoAdvectionDF::setCellVertexValues(
    const label celli,
...
void Foam::isoAdvectionDF::normaliseAndSmooth(
...
void Foam::isoAdvectionDF::setDownwindFaces(
    const label celli,
    DynamicLabelList& downwindFaces) const
{
...
void Foam::isoAdvectionDF::limitFluxes()
{
    DebugInFunction << endl;
    ...
    //Info << "isoAdvectionDF: Before conservative bounding: min(alpha) = "
    ...
    for (label n = 0; n < nAlphaBounds_; n++)
    {
        ...
        if (minAlpha < -aTol) // Note: tolerances
        {
            ...
            forAll(correctedFaces, fi)
            {
                ...
                scalar tVal = phi * dt - dVcorr;
                setFaceValue(dVf_, facei, tVal);
            }
...
void Foam::isoAdvectionDF::boundFromAbove(
...
Foam::scalar Foam::isoAdvectionDF::netFlux(
...
void Foam::isoAdvectionDF::syncProcPatches(
    surfaceScalarField& dVf,
    const surfaceScalarField& phi)
{
    const polyBoundaryMesh& patches = mesh_.boundaryMesh();
...
void Foam::isoAdvectionDF::checkIfOnProcPatch(const label facei)
{
    if (!mesh_.isInternalFace(facei))
    {
...
void Foam::isoAdvectionDF::advect()
{
    DebugInFunction << endl;
    ...
    // Do the isoAdvectionDF on surface cells
    timeIntegratedFlux(dVf_);
    ...
    //Info << "isoAdvectionDF: After conservative bounding: min(alpha) = "
    //     << minAlpha << ", max(alpha) = 1 + " << maxAlphaMinus1 << endl;
    ...
    //Info << "isoAdvectionDF: time consumption = "
    //    << label(100*advectionTime_/(mesh_.time().elapsedCpuTime() + SMALL))
    //    << "%" << endl;
}

void Foam::isoAdvectionDF::applyBruteForceBounding()
{
...
void Foam::isoAdvectionDF::writeSurfaceCells() const
{
...
void Foam::isoAdvectionDF::writeBoundedCells() const
{
    if (!mesh_.time().writeTime())
        return;
...
void Foam::isoAdvectionDF::writeIsoFaces(
    const DynamicList<List<point>>& faces) const
{
    ...
    if (Pstream::parRun())
    {
        ...
        if (Pstream::master())
        {
            ...
            Info << nl << "isoAdvectionDF: writing iso faces to file: "
                 << os.name() << nl << endl;
