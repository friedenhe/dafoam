    ...
    const scalar psiMin)
{
    ...
    const dictionary& MULESDFcontrols = mesh.solverDict(psi.name());

    const label nLimiterIter(
        MULESDFcontrols.lookupOrDefault<label>("nLimiterIter", 3));

    const scalar smoothLimiter(
        MULESDFcontrols.lookupOrDefault<scalar>("smoothLimiter", 0));

    const scalar extremaCoeff(
        MULESDFcontrols.lookupOrDefault<scalar>("extremaCoeff", 0));
