...
defineTypeNameAndDebug(DAInterIsoFoam, 0);
addToRunTimeSelectionTable(DASolver, DAInterIsoFoam, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DAInterIsoFoam::DAInterIsoFoam(
    char* argsAll,
    PyObject* pyOptions)
    : DASolver(argsAll, pyOptions),
      ...
      advectorPtr_(nullptr)
{
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

void DAInterIsoFoam::initSolver()
{
    /*
    Description:
    ...
    */
    ...
    Info << "Initializing fields for DAInterIsoFoam" << endl;
    Time& runTime = runTimePtr_();
    fvMesh& mesh = meshPtr_();

#include "createPimpleControlPython.H"
#include "createFieldsInterIso.H"
...
label DAInterIsoFoam::solvePrimal()
{
    /*
    Description:
...
#include "alphaEqnInterIso.H"
            rho == alpha1* rho1 + alpha2* rho2;

            mixture.correct();

#include "UEqnInterIso.H"

            // --- Pressure corrector loop
            while (pimple.correct())
            {
#include "pEqnInterIso.H"
            }
